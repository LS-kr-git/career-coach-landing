...
온보딩 1단계의 **대분류 18 · 중분류 174** 는 우리가 지어낸 문구가 아니라 **직행(zighang) 의 depth 1 / depth 2** 다.
...
- **`crawler/roles.py` 의 `DEPTH_ONE` 과 짝이다.** 한쪽만 늘리면 조용히 어긋난다.
  직행 대분류는 실제로 26개고, 아직 안 실은 8개는 그쪽 `DEPTH_ONE_UNLISTED` 에 있다
