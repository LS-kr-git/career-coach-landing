...
**모든 페이지 (`page-audit.mjs`)** — 깨진 링크 · 경로 대소문자 · head 필수 태그 · 자리표시자 · `http://` 리소스 · `github.io` 하드코딩 · 블록 태그 균형 · CNAME 형식

**랜딩만 (`audit.mjs`)** — 문구 · 역방향(웹 전용 문구) · 부분 일치 · 확정값 21건 · 이미지 sha256 6종 · 타이포(12조합 / 18선택자) · 타이포 스냅샷 신선도
