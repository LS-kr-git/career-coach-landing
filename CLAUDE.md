...
- 근거(2025~26 관례): 최소 탭 44×44(HIG·Material·WCAG), 주요 CTA 48~56 편안 + 모바일 풀폭. 두께는 대부분 웹이 SemiBold(600)이지만 **주요·전환 CTA·한국 소비자앱(토스·당근·카카오 계열)은 Bold(700)** 가 흔하다 — 위계를 위해 700 을 쓴다(2026-08-03 사용자 확정, 600→700). 크기 15 는 `text/button` 값이다.
  - **피그마 CTA 두께 700 — 브라우저에서만 된다** (플러그인 ❌ / figma.com 웹앱 ✅). 플러그인(`use_figma`)은 Pretendard 를 `loadFontAsync` 못 해서 굵기를 못 바꾼다. 하지만 **사용자가 로그인된 figma.com 웹앱은 Pretendard 가 있어 된다** — Claude in Chrome 으로 직접 바꾼다. 방법: 텍스트 노드 선택 → Typography 스타일 칩 오른쪽 **Detach style**(체인 끊기 아이콘) → 개별 폰트 컨트롤이 뜨면 **Weight 드롭다운 = Bold** 선택. (`text/button`(15/600) 공유 스타일 하나뿐이고 15/700 스타일이 없어 detach 가 필요하다. detach 해도 **문구는 그대로**라 docs-audit(문구 동기화)엔 영향 없다.) 굵기만 로컬 오버라이드하려면 setTextStyleId 대신 이 방법.
    - **레터 CTA `364:2502`("내일 아침에도 받아보기") = Pretendard 15/700 Bold(detached) 로 반영 완료** (2026-08-03, 브라우저). 웹(letter.html, `--cta-fw:700`)과 일치.
    - 필요하면 나중에 `text/button-strong`(15/700) 공유 스타일을 추가해 CTA 들에 붙이는 게 더 깔끔하다(현재 피그마 내 비랜딩 CTA 는 이 하나뿐이라 detach 로 충분). 랜딩 CTA `6:368` 은 17/600 예외 — 건드리지 않는다.
