 ...
 * 이 규칙이 의미가 있다.
 */
// 🔴 이 검사는 매칭 전에 <script> 를 뗀다(:289). 그래서 금액을 JS 로 옮기면 이 목록과 무관하게
//    안 걸린다 — 2026-09-05 에 mypage/billing 이 그 자리에 들어갔다. 면제를 여기 적으면
//    사각지대와 면제가 겹쳐 **나중에 그 파일 마크업에 진짜 금액이 새어도 막을 것이 없어진다.**
//    그래서 적지 않는다. 마크업 쪽 방어는 살려 두고, JS 구멍은 아는 채로 남긴다.
const PRICE_OK = new Set([]);          // 결제(체크아웃) 화면이 생기면 여기에 추가
