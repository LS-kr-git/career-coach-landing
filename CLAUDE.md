...
## 온보딩 1단계 직군 목록 — `taxonomy.json` 이 기준, HTML 은 생성물 (2026-08-04 신설)

온보딩 1단계의 **대분류 25 · 중분류 296** 은 우리가 지어낸 문구가 아니라 **직행(zighang) 의 depth 1 / depth 2** 다.
사용자가 고른 값이 그대로 공고 조회 파라미터(`depthOnes` / `depthTwos`)가 돼야 선택 → 저장 → 조회가 이어진다.

```
tools/roles/taxonomy.json   ← 단일 기준. code(조회 표기) / label(화면 표기)
tools/roles/build.mjs       ← onboarding/1/index.html 의 .scroll 블록을 다시 만든다
node tools/roles/build.mjs           # 반영
node tools/roles/build.mjs --check   # 훅이 부르는 드리프트 검사 (1.5겹)
```

- **HTML 을 손으로 고치지 않는다.** 훅이 막는다. 문구·분류를 바꾸려면 `taxonomy.json` 을 고치고 생성기를 돌린다.
- **`code` 와 `label` 을 섞지 않는다.** 직행은 구분자가 `_` 다. 화면 표기(`·`)로 조회하면 **에러 없이 0건**이 온다.
  칩에는 `data-d2="퍼포먼스마케팅"`, 아코디언에는 `data-d1="마케팅_광고_홍보"` 가 붙어 있다 — 저장·조회는 이 값을 쓴다.
- **`crawler/roles.py` 의 `DEPTH_ONE` 과 짝이다.** 한쪽만 고치면 조용히 어긋난다.
  (근거: `career-coach/docs/검증/직행-직무분류-전수.md`)
- **중분류 label 을 예쁘게 다듬지 않는다.** `기타마케팅` → `기타`, `임베디드소프트웨어` → `임베디드SW` 로
  줄였다가 되돌렸다(2026-08-04). 줄이는 순간 `code = label.replace('·','_')` 관계가 끊겨
  code 표를 손으로 관리하게 되고, 손으로 관리하는 표는 반드시 어긋난다.
  **대분류 label 만 예외** — 직행의 짧은 사이드바 표기(`마케팅·광고`) 대신 편 형태(`마케팅·광고·홍보`)를 쓴다.
  접힌 헤더는 스캔 대상이라 이름만으로 안에 뭐가 있는지 알아야 한다. code 는 직행 값 그대로다.
- **목록을 다시 뽑을 때는 API 표본이 아니라 `zighang.com/recruitment` 의 필터 > 직무 패널을 읽는다.**
  API 응답 집계는 '지금 공고가 있는 직무' 만 보여 준다 — 2026-08-04 에 그 방식으로 중분류 96개를 놓쳤다.
- **시작 상태는 전부 접힘 · 선택 0** 이다(피그마 `278:2693`). 2026-08-04 이전 웹은 13개가 미리 켜져 있었다 — 되돌리지 말 것.
- 피그마 쪽 대응 프레임: `278:2693`(시작) · `278:2404`(펼침·미선택) · `275:2404`(선택됨).
  세 프레임 모두 같은 목록을 담는다. 목록을 바꾸면 셋 다 고친다.
