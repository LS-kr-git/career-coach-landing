...
## 조용히 통과하던 자리 넷을 막았다 (2026-08-20)

`audit.mjs` 는 준비물이 갖춰져야 도는 검수라, **준비물이 반쯤 빠진 채로도 초록이 뜨는** 자리가 넷 있었다.
넷 다 종료코드 0 이고 화면에 아무 표시가 없어서, 사람이 훑으면 "차이 없음" 과 구분되지 않았다.

| 무엇이 조용했나 | 지금 |
|---|---|
| **빈 스타일 덤프** `[]` 를 주면 "피그마 노드 0개와 대조 (차이 0건)" 으로 통과 (같은 조건의 타이포 덤프는 exit 1 이라 두 경로가 비대칭이었다) | 대조한 양을 **속성 단위**로 세어 `속성 P/Q · 항목 N/M` 으로 늘 찍고, **하나도 못 봤으면** 🎨 조치 필요 |
| **메타 XML 을 줄이다 `<text>` 줄을 흘리면** 문구 대조가 아무것도 안 돌면서 웹 문구가 전부 `ℹ️ 웹전용` 으로 빠진다 (실측 "총 87건 · 조치 필요 0건") | `figma-type.json` 이 아는 텍스트 노드와 대조해 **커버리지**를 찍고, 70% 미만이면 「메타 XML 축약 의심」 으로 막는다 |
| **덤프 신선도가 `mtime` 만** 봐서 `cp`·`touch`·checkout 으로 시각만 새로 찍힌 낡은 덤프가 통과 | 타이포·스타일 덤프는 `{ dumpedAt, nodes }` 형태로 뽑고, **mtime 과 dumpedAt 중 더 낡은 쪽**을 나이로 쓴다. `dumpedAt` 이 없으면 그 사실을 조치 필요로 올린다 |
| **`CC_META_MAX_AGE`** 로 한도를 늘리거나 숫자가 아닌 값을 넣으면 45분 관문이 흔적 없이 꺼진다 (`ageMin > NaN` 은 항상 거짓) | 한도와 그 출처를 **항상 출력에 찍고**, 기본값보다 크거나 양수가 아니면 조치 필요로 올린다 |

한도·나이 판정은 `dump-age.mjs` 한 곳에 있고 `audit.mjs`·`tree-audit.mjs` 가 같이 쓴다.
앞 판은 두 파일이 각자 구현해서 `dumpedAt` 파싱 조건이 달랐고, 45분이 양쪽에 따로 박혀 있어
`CC_META_MAX_AGE` 가 `audit` 만 움직였다.

### 왜 "부분 미대조" 는 막지 않나

스타일 덤프 스니펫이 만드는 키는 `fill·textFill·stroke·sw·radius·shadow·pad·gap·h` 뿐인데,
`figma-style.json` 이 쓰는 키에는 `indent`·`w`·`segment` 처럼 **그 스니펫이 만들 수 없는 것**도 있다.
그래서 **정상 덤프로도 전부 대조되지는 않는다** — 2026-08-20 실측으로 `속성 41/48 · 항목 21/25` 다.
여기서 부분 누락까지 막으면 `index.html` 을 고친 **모든 푸시가 정상 덤프로도 선다.**
막는 대신 **비율을 늘 찍는다.** 그 숫자가 정상값(41/48)에서 내려가면 사람이 §6 표에서 본다.

### 메타 XML 커버리지의 기준값은 어디서 오나

숫자를 박으면 낡으므로 `figma-type.json` 의 `groups[].figmaNodes` 를 쓴다. 인스턴스 내부(`I…` id)는
`get_metadata` 가 원래 펼치지 않으므로 빼고, `ignoreNodes` 도 뺀다. 세는 것은 **문구가 실린 노드**다 —
id 만 세면 `name=""` 으로 문구만 비운 XML 이 100% 로 통과한다(실측).
**2026-08-20 실측: 정상 XML 은 79/81 = 98%** 라 한도 70% 는 크게 벌어져 있다.
`figma-type.json` 이 없거나 `groups` 가 비면 **판정 불가로 막는다** — 기준이 없으면 어떤 XML 이든 통과하므로.

열 방향 확인됨(2026-08-20). ①②③은 **CLAUDE.md 의 스니펫을 피그마에서 그대로 돌린 실제 덤프**로 쟀다.

| 준 것 | 나온 것 |
|---|---|
| ① 정상 XML + 실제 스니펫 덤프 | 커버리지 98% · `속성 41/48 · 항목 21/25` · 조치 신호 없음 |
| ② 20줄로 줄인 XML | 커버리지 25% → ❌ 축약 의심 |
| ③ id 는 그대로 두고 `name=""` 으로 문구만 비운 XML | 커버리지 0% → ❌ 축약 의심 |
| ④ 빈 스타일 덤프 | `속성 0/48` → 🎨 신선도 대조 못 함 |
| ⑤ `dumpedAt` 없는 덤프 | 🕰️ 「dumpedAt 이 없음」 (mtime 만 봤다는 사실이 남는다) |
| ⑥ `dumpedAt` 을 3시간 전으로 적고 파일은 방금 `touch` | 🕰️ 「180분 전 덤프」 (mtime 만 봤다면 통과했을 것) |
| ⑦ 같은 덤프를 `.json` 이 **아닌** 이름으로 저장 (훅이 `FIGMA_TREE`·`FIGMA_STYLE` 로 임의 경로를 받는다) | 「180분 전 덤프」 — 이름이 아니라 **JSON 으로 파싱되는가**로 가른다 |
| ⑧ 메타 XML 을 `.txt` 이름으로 저장 | 「(mtime만)」 — 없는 `dumpedAt` 을 트집잡지 않는다 (⑦의 반대 방향) |
| ⑨ 없는 40자 sha 를 `live-plan` 기준으로 줌 | ❌ exit 2 + `git fetch` 안내 (맨 `rev-parse` 는 없는 객체도 그대로 돌려준다) |
| ⑩ `tree-audit` 을 `--strikt` 로 오타 | ❌ exit 2 「모르는 인자입니다」 — 조용히 옛 동작으로 돌아가지 않는다 |

`tree-audit` 머리말은 **`모드 strict` / `모드 기본` 을 늘 찍는다.** 유일한 호출자가 저장소 밖이라
grep 으로 확인이 안 되므로, 출력에 남지 않으면 "돌렸는데 깨끗한 것" 과 "관문이 꺼진 것" 이 구분되지 않는다.

**이 저장소에는 `audit.mjs` 를 도는 자동 테스트가 없다** — 위 열 가지는 손으로 만든 입력과 실제 덤프로 확인한 것이다.
...
node tools/figma-audit/tree-audit.mjs figma_tree.json --strict   # 🚫 미실행도 실패로 센다
```

### `--strict` — 미실행을 실패로 세는 호출 (2026-08-20)

이 검수는 **모든 푸시에서** 도는데 덤프는 세션마다 뽑는 물건이라, 미실행(🚫)을 막힘으로 두면
평소 작업이 통째로 선다. 그래서 기본값은 종료코드 0 이다 — 그런데 그러면 **덤프를 반드시 뽑고
들어오는 호출**(예약 점검)에서도 미실행이 초록으로 통과한다.

`--strict` 는 그 계약을 **부르는 쪽이 선언**하게 한다. 스크립트에 도피로를 만드는 것이 아니라
반대 방향이다 — 평소 훅은 그대로 두고, 덤프가 있어야 마땅한 호출만 엄격해진다.
**호출자는 저장소 밖에 있다** — 예약 점검 「[3주] 라이브↔피그마 대조」 의 프롬프트 §A-1.5 가
이 플래그로 부른다. 저장소 안에서 grep 해도 안 나오니 "안 쓰는 플래그" 로 오해하지 말 것.
푸시 훅(`tools/hooks/pre-push`)은 **일부러** 이 플래그 없이 부른다.
...
## live-plan.mjs — 라이브 대조 계획

```
node tools/figma-audit/live-plan.mjs "$BASE" HEAD     # ← 따옴표 필수
```

커밋 범위에서 "어떤 페이지를 무슨 문자열로 확인할지" 를 뽑는다. 사람이 대조 문자열을 눈으로
고르다 그 페이지에 없는 문구를 골라 사고로 오인할 뻔한 것이 만든 이유다(2026-08-08).

**2026-08-20 에 조용한 자리 둘을 막았다.**

- **기준과 대상이 같은 커밋이면 exit 2 로 멈춘다.** 전에는 diff 가 비어 "변경 파일 0개 · HTML 변경 없음"
  을 찍고 통과했는데, 그 출력이 **진짜로 변경이 없던 주와 글자 하나 다르지 않았다.** 셸에서 따옴표를
  빼 `$BASE` 가 비면 인자가 한 칸 밀려(`live-plan.mjs HEAD`) 정확히 이 상태가 된다.
- **요약줄에 robots 차단(🔒) 건수를 넣었다.** 전에는 "대조 가능 N개 · 확인 불가 N개" 뿐이라
  🔒 로 분류된 페이지가 표에서 통째로 사라졌다 — 실측으로 7건이 사라진 적이 있다.
  지금은 세 숫자의 합과 HTML 개수를 같이 찍어 빠진 것이 눈에 남는다.
...
### `webScriptText` — `<script>` 안 상수를 웹 문구로 읽는다 (2026-09-15 신설)

이 검수는 웹을 훑기 전에 `<script>` 를 통째로 걷어낸다. 그래야 코드 토막이 문구로 안 샌다.
그런데 **화면에 뜨는 한국어를 상수에 담아 JS 로 그리는 페이지**가 있다 — `insight/index.html` 의
`LOCKED`(잠김 문구 5종) · `SAVED_TOAST`(소장 알림 4줄) · `MSG`(안내문 3종). 그 문구는 마크업에
없으니 전부 `jsRenderedText` 로 면제돼 있었고, **면제된 문구는 피그마→웹 대조를 안 받는다.**

> 2026-09-15 변이 실측: 프레임 `2058:751` 의 문구를 `texts`·`jsRenderedText` 양쪽에서 바꿔도
> `docs-audit` 이 0건이었다. 프레임을 11장 등록해 놓고도 문구는 아무도 안 보고 있었다.

스냅샷의 페이지 항목에 상수 **이름**을 적으면 그 상수의 초기화식 안에 있는 문자열만 골라
웹 문구 더미에 넣는다:

```json
"webScriptText": ["LOCKED", "SAVED_TOAST", "MSG"]
```

`selectionCap` 과 같은 규칙이다 — **냄새로 찾지 않고 선언을 강제한다.**

| 상황 | 결과 |
|---|---|
| 선언한 이름을 스크립트에서 못 찾음 | ❌ `선언한 상수 없음` (막는다) |
| 상수는 있는데 문자열이 0개 | ❌ `상수에 문구 없음` (막는다) |
| 상수 안에 `${…}` 백틱 문자열 | ❌ `상수에 치환 문자열` — 그 한 줄만 상수 밖으로 빼라 |
| 선언을 **빠뜨림** (아래 조건일 때만) | ❌ `선언을 빠뜨림` (막는다) |

**선언을 빠뜨리는 경로도 막는다** (2026-09-15 · `selectionCap` 의 `상한 선언 없음` 과 같은 모양).
`webScriptText` 는 사람이 적는 값이라, 새 상수를 만들고 이름을 안 적으면 그 문구는 조용히 대조
밖에 남는다 — 검사가 는 것처럼 보이지만 실제로는 안 는다.

다만 **자유로 고칠 수 있을 때만** 막는다: 그 상수의 한국어 문구가 **이미 전부 피그마에 있을 때**다.
그때 고치는 법은 이름 한 줄을 적는 것뿐이라 화면을 만들 일이 없다. 피그마에 없는 문구가 섞인
상수까지 막으면 「승인 안 된 화면을 그려라」가 되어, **남이 작업 중인 화면에서 푸시가 서 버린다.**
그런 자리는 막는 대신 스냅샷에 사유를 적어 남긴다(예: 결제 화면이 없어 한쪽 갈래를 못 그리는 자리).

후보는 **스칼라 선언까지** 본다 — `const X = { }` · `[ ]` 뿐 아니라 `const X = '문구'` 도 잡는다.
좁게 잡으면 「선언을 빠뜨림」이 조용히 0건이 되므로, 그 정규식(`CONST_DECL`)도 자가검사가 고정한다.
방아쇠는 **네 글자 이상**의 한국어 문구만 센다 — 대조가 부분 문자열 포함이라 `'월'` 같은 한 글자는
거의 늘 「피그마에 있다」가 되어 목업 데이터까지 끌어들인다(웹→피그마 대조가 쓰는 문턱과 같다).

읽는 범위는 좁다. **객체 키는 건너뛴다**(바로 뒤가 `:` 인 리터럴). 배열·중첩 객체는 그대로 읽고,
**선언 순서를 지켜 넣는다** — 피그마 한 노드가 줄바꿈으로 이어 붙인 두 줄(알림 설명문)이
합쳐진 웹 문구 안에서 연속으로 만나야 대조에 걸리기 때문이다.

훑개는 **주석을 뗀다.** 안 떼면 두 가지로 틀린다 — 둘 다 2026-09-15 검사관이 변이로 뚫었다:

- 주석 속 아포스트로피 하나(`// don't touch`)가 따옴표 상태를 뒤집어 초기화식이 파일 끝까지
  늘어나고, 쏟아지는 오탐 속에 진짜 드리프트가 묻힌다
- **화면 문구를 지워도 같은 문장이 주석에 남아 있으면 주석 사본이 대신 채워 초록이 된다**
  (CLAUDE.md 「테스트에 대한 규칙」 5번이 여섯 번 물렸다고 적은 그 모양이다)

이 배선을 고정하는 **자가검사가 docs-audit 안에 있고 조건 없이 매번 돈다**(`scanSelfTest`).
깨지면 「훑개 자가검사 실패」로 막는다 — 이 검사가 깨질 때의 증상이 「조용히 초록」이기 때문이다.
고정한 자리 여덟은 위 두 함정, 삼항 두 갈래, 배열, 줄바꿈 이스케이프, 다음 선언으로 안 넘어가는 것,
백틱을 안 읽는 것이다.

⚠️ **정규식 리터럴은 못 읽는다** — 그 안의 따옴표가 상태를 뒤집는다. 문구 상수에 정규식을 넣지 마라.
백틱은 **치환(`${…}`)이 없을 때만** 보통 문자열로 읽는다.

⚠️ **삼항으로 갈리는 문구는 묶지 마라.** `CHECKOUT_URL ? '구독 시작하기' : '구독 준비 중이에요'`
같은 자리는 두 값 중 한쪽만 피그마에 있는 경우가 많다. **두 갈래 다 읽으므로** 없는 쪽이
「웹 전용」으로 막히고, 그걸 피하려고 승인 안 된 화면을 그리게 된다. 그런 자리는
`jsRenderedText` 에 남기고 **왜 남겼는지를 스냅샷에 적는다.**
(앞 갈래를 객체 키로 오인해 **조용히 버리던** 판이 있었다 — 2026-09-15 검사관 ①. 지금은 둘 다 읽는다.)
