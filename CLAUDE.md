...
탐색은 서브에이전트에 맡기고 결론만 받는다. 어디에 있나 / 몇 곳에서 하나 / 이미 있나 는 원문을 이 대화에 들이지 않는다. 지금 고칠 파일만 직접 읽는다.
...
## 0. 클론 직후 두 줄 — 푸시 훅 켜기 + 얕은 클론 풀기
```
git config core.hooksPath tools/hooks
git fetch --unshallow          # 얕게 클론됐다면. 이미 깊으면 아무 일도 안 한다
```

🔴 **둘째 줄을 빼면 `docs-audit`(2겹a)이 통째로 빨개진다** (2026-08-14 실측).
얕은 클론에서는 파일별 커밋 날짜를 못 읽어 모든 경로가 tip 커밋으로 나오고, 그러면
검수가 대상 8개 전부를 `🕗 낡음 — 판정 불가` 로 올린다. **스냅샷은 멀쩡한데** 그렇게 뜬다.

한 세션이 실제로 이걸 "온보딩 피그마 스냅샷이 낡아 랜딩 푸시를 막고 있다" 로 오진하고
사용자에게 보고했다. `--unshallow` 뒤 재측정하니 **차이 없음**이었다. career-coach 의
`c2c212e` 가 CI 쪽에서 같은 원인을 이미 진단해 뒀는데(그쪽은 `actions/checkout` 에
`fetch-depth: 0`), 로컬 클론 쪽에는 그 지식이 안 적혀 있었다.
...
코드에는 여섯 개가 있었다. 1.2겹·1.5겹이 빠져 있었다. 2026-09-12 에 1.6겹이 늘었다.)
...
| 1.6 | `tools/mypage-data-check.mjs` | 모든 푸시 (node 있을 때) | 마이페이지 데이터 층(`assets/mypage-data.js`) ↔ 정적 마크업 (docs-audit 이 못 보는 JS 렌더 값). CI 에는 아직 없다 |
...
### 🔴 node 가 없는 PC 에서는 1~2겹이 안 돈다 — 막지는 않는다 (2026-08-13)

1~2겹은 전부 node 로 돈다. 그런데 **사용자 PC 3대에 node 가 없을 수 있다**
(2026-08-11 실측: `DESKTOP-F7S1OEL`. 정본은 career-coach `docs/기획/피그마-웹-동기화.md`
「랜딩 pre-push 훅은 node 를 쓴다」).
...
피그마 덤프도 그때는 **요구하지 않는다** — 못 돌릴 검사 때문에 푸시를 막으면 다시
`--no-verify` 를 부른다.

**건너뛴 것 대부분은 사라지지 않는다.** 검수 **셋**(1겹 page-audit · 1.5겹 직군 목록 ·
2겹a 문구 대조)이 `.github/workflows/landing-audit.yml` 에서 리눅스로 다시 돈다.
훅을 느슨하게 한 것이 아니라 **옮긴 것**이다.

🔴 **CI 로 못 옮긴 것이 둘 있다. 라이브 피그마 덤프가 필요해서다.**

| 검사 | 왜 CI 에 없나 |
|---|---|
| 2겹b `audit.mjs` (랜딩 피그마 대조) | 덤프 3개가 필요하고 CI 에서 `use_figma` 를 못 부른다. **훅에만 있다** |
| 1.2겹 `tree-audit.mjs` (고아 노드) | 덤프 없이 부르면 `🚫 검수 미실행` 을 찍고 **종료코드 0** — CI 에 넣으면 항상 초록이라 뺐다 |

그래서 **node 없는 PC 에서 `index.html`/`assets` 를 고쳤다면, node 가 있는 곳에서 한 번
대조하고 미는 것이 좋다.** 훅이 그 사실을 푸시할 때 말해 준다.
...
1겹은 페이지를 새로 추가해도 설정을 고칠 필요 없이 자동으로 대상에 들어온다.
(여기에 "준비물이 없어 어떤 세션에서든 무조건 돈다" 고 적혀 있었다 — **틀렸다. node 가 필요하다.** 위 절 참조.)
...
2겹b 는 피그마 덤프 **3개**(메타·타이포·스타일)가 있어야 한다. 훅이 직접 막는 것은 `figma_meta.xml`
하나지만, 나머지 둘이 없으면 `audit.mjs` 가 `TYPE`·`STYLE` 등급을 올려 **종료코드 1 로 막는다.**
"없어도 경고만" 이 아니다 (2026-08-13 정정 — 여기가 "2개" 라고 적고 있었다).

⚠️ 덤프 세 개는 **저장소 밖**에서 찾는다 — `$FIGMA_*` → 저장소 루트 → 상위 디렉터리 → `$HOME`.
샌드박스에서는 `/home/claude` 에 있어서 통과하는데 PC 에서는 없어서 막힐 수 있다.
...
🔴 **① 앞에 ⓪ 가 있다 — 시안은 HTML 모바일 목업에서 돌리고 컨펌된 것만 피그마로 옮긴다** (2026-09-05 사용자 확정 · 2026-09-25 캔버스 → HTML 목업).
화면 안 내용을 보일 때는 **항상 HTML 목업**(구독자 모바일 · 어드민 PC)이다 — 정본은 `career-coach` 저장소 `CLAUDE.md` 의 「📱 화면 안 내용은 항상 HTML 목업으로 보여준다」 절.
정본은 `career-coach` 저장소 `CLAUDE.md` 의 「구독자 화면은 시안을 HTML 모바일 목업에서 돌리고 확정본만
...
## 넓은 화면에서는 좌우 회색 바탕 + 카드 그림자 (2026-09-24 사용자 확정 · 모든 페이지 · 검수가 강제)

> "PC 로 볼 때 좌우에 회색 배경이 깔리고 경계는 그림자 처리 — 앞으로 모든 세션과 모든 페이지에서 동일하게.
>  기준은 현재의 랜딩페이지."

값의 정본은 `index.html` 의 「넓은 화면 카드 어포던스」 블록이다. 새 페이지는 그 블록을 **그대로** 옮긴다
(카드 역할 요소가 `.page` 가 아니면 선택자만 바꾼다 — 온보딩은 `.board`).
`page-audit.mjs` 「넓은 화면 바탕」 이 `ops/` 를 뺀 모든 페이지에서 `@media (min-width:451px)` 안의
`body{background:var(--gray-100)}` 와 그림자 값을 보고, 없으면 푸시를 막는다.
2026-09-24 에 마이페이지 여섯 장만 이 블록이 없어 PC 에서 좌우가 흰색이었다.
...
- **`npm i playwright` 를 먼저 한다. 새 컨테이너에서는 준비물이 없다.** 타이포·시각 스타일·반응형
  세 검수는 브라우저 실측이라 playwright 가 없으면 스스로 건너뛴다. 2026-08-08 까지는 **건너뛴 사실이
  조치 필요로 안 올라가서** 4겹 중 문구 1겹만 돌고도 "조치 필요 0건" 이 뜨고 종료코드 0으로
  pre-push 를 통과했다 — 목록에 없으면 검사가 없고, 검사가 없으니 통과였던 그 유형이다.
  지금은 `🚫 검수 미실행` 로 **막힌다**(신선도 미확인과 같은 원칙). 일부러 건너뛰는 플래그는 두지 않았다.
- "차이 없음" 이 나와야 푸시한다.
...
```
pre-push 2겹(a)에서 **`*.html` 이 하나라도 바뀐 푸시**마다 자동으로 돈다.
(2026-08-13 정정 — 여기가 파일 셋을 손으로 나열하고 있었다. 그 목록이 낡아서 signup/onboarding 이
대조에서 빠졌던 것이 2026-08-07 사고이고, 훅은 그때 이미 `*.html` 로 바뀌었다.)
...
5. **커밋 직전에 2번 스니펫을 한 번 더 돌린다. 길이·시간과 무관하게 늘 한다.**
   ...
   이 한 번이 그 구간을 덮는 유일한 장치다.
   - 그 사이 피그마가 안 바뀌었으면 결과가 똑같아 **diff 가 아예 안 생긴다.** 비용은 읽기 호출 하나다.
   - 달라졌으면 **새 덤프가 기준**이다. 웹을 거기 맞추고, **남의 변경이 섞여 들어왔다는 사실을
     결과 보고에 쓴다** (프로젝트 지침의 "공유 상태를 바꾸기 직전에 세션 충돌을 확인한다" 그대로).
     내 작업 범위 밖의 문구가 딸려 오면 조용히 흡수하지 말고 그것도 보고한다.
...
| 기준 | 값 | 왜 |
|---|---|---|
| 대분류 | 주당 신규 **5건 미만이면 숨김** | 2026-08-04 에 **10 → 5 로 완화**했다 — 10 은 게임·증권·교육·서비스·공공복지·의료보건까지 잘라서 "그 직군 사람은 온보딩에서 그냥 이탈" 하는 쪽이 더 컸다. 정본 값은 `build.mjs` 의 `MIN_D1_PER_WEEK` |
| 중분류 | 30일 신규 **0건이면 숨김** | 한 달 내내 새 공고가 없었다 = 골라도 아무것도 안 온다 |

> 🔴 **이 표는 2026-08-16 까지 「주 10건」으로 적혀 있었다.** 코드는 08-04 부터 5 였다 —
> 값을 여기서 읽지 말고 `build.mjs` 에서 읽어라. 이 문서는 **왜** 그 값인지만 말한다.

### ⏸ 결정됐지만 아직 반영 안 됨 — 두 축 예외 (2026-08-16 사용자 확정 · 피그마 대기)

**대분류 게이트가 공고 유입 하나만 보고 있다.** 우리 제품은 아티클(주 5일)과 공고(주 1일)
두 축인데(career-coach `docs/기획/발송-검수-설계.md` 「정체성 한 줄」) 이 게이트는 공고 축만 센다.
「운송·배송」은 공고가 주 4.7건이라 숨어 있지만 「물류·공급망」 전용 트랙이 붙어 매주 아티클
3편이 나간다 — 그 직군 사람은 가입 화면에서 자기 직무를 못 찾는다.

- **"인사이트가 오면 안 숨긴다" 로 쓰면 게이트가 통째로 무의미해진다** — 공통 트랙
  (커리어·이직 · AI로 일하기 · 일하는 법)은 **전 직군이** 받기 때문이다. 가르는 것은
  「인사이트가 오는가」가 아니라 **「자기 직군 전용 트랙이 있는가」** 다.
- 실측 효과(2026-08-16): 새로 보임 **운송·배송 1개** · 사라짐 **0개** · 순서 변화 없음.
- **왜 아직 안 넣었나**: 목록이 바뀌면 피그마 프레임 셋(`278:2693`·`278:2404`·`275:2404`)이
  같이 가야 하고, 안 가면 `docs-audit`(2겹a)이 푸시를 막는다. 그런데 이 파일의 폰트
  `Pretendard Variable` 은 **피그마 클라우드 렌더러에 없다**(2026-08-16 실측:
  `listAvailableFontsAsync` 8,927개 중 0개 · 기존 노드 전부 `hasMissingFont=true`).
  원격 MCP 로 텍스트를 만들면 대체 폰트 폭이 파일에 박힌다 —
  career-coach `docs/기획/피그마-웹-동기화.md` 의 「절대 하지 말 것」 이 금지하는 그것이다.
- **다음 순서**: 크롬이 붙고 Pretendard 가 잡히는 기기에서 ① 피그마 프레임 셋에 행 추가 →
  ② 폰트 재계산 + 폭 검증 → ③ 스냅샷 재수집 → ④ `build.mjs` 패치 적용 후 `node tools/roles/build.mjs`.
  ④ 패치는 검증까지 끝나 있다(오타 변이 실험 포함) — 프로젝트 문서
  `claude/패치-⑭-직무노출-두축예외-2026-08-16.md`.
...
**⚠️ 이 경로는 푸시 훅을 건너뛴다.** 0겹(배포 쿨다운)·1겹(page-audit)·**1.2겹(tree-audit)**·1.5겹(직군 목록)·2겹(피그마 대조)이
안 돈다. (2026-08-13 정정 — 1.2겹이 빠져 있었다. 다른 절이 "tree-audit 은 항상 돈다" 고 적고 있어서
API 로 미는 세션이 그 검사가 여전히 도는 줄 알았다.) **올리기 전에 로컬에서 손으로 다 돌리고**, 결과를 PR 본문에 적는다.
