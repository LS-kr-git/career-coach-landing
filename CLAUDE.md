...
## CTA 규칙 — 모든 페이지 공통 (2026-08-03 확정)

버튼(주요 CTA)의 세로 길이·라운드·글자가 페이지마다 달라(높이 52~60 · 라운드 12~14 · 글자 15~17) "제각각·길다"는 지적이 있었다. 아래로 통일한다.

**토큰 — 각 페이지 `:root` 에 동일하게 넣는다:**
```
--cta-h:54px; --cta-radius:14px; --cta-fs:15px; --cta-fw:600;
```
- **높이 54 · 라운드 14 · 글자 15/600(=피그마 `text/button`) · 모바일 풀폭.** 라운드 14는 카드와 같은 값이라 시스템이 하나로 붙는다. 60→54 로 낮춰 "길다"를 없앤다.
- 근거(2025~26 관례): 최소 탭 44×44(HIG·Material·WCAG), 주요 CTA 48~56 편안 + 모바일 풀폭. 15/600 은 디자인 시스템의 `text/button` 스타일이다(16/600 토큰은 없어 15 로 맞춘다).
...
- 적용됨: `signup` · `letter` · `onboarding/1·2·3`.
- **랜딩(index.html) CTA는 예외 절차**: 글자 노드 `6:368` 이 타이포 검수 대상이라, 글자를 15 로 내리려면 **피그마 6:368 에 `text/button` 을 입히고 → 3종 덤프 재생성 → audit 통과 → 웹을 맞춘다**(위 "랜딩을 건드렸다면" 절차). 높이·라운드만 바꿔도 index.html 변경이라 2겹 덤프가 필요하다. 현재 랜딩 CTA는 히어로로 `text/h3`(17/600) 유지 중 — 통일하려면 이 절차를 따른다.
