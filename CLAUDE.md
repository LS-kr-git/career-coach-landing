...
### 🔴 피그마는 크롬 피그마(figma.com)로 한다 — 필수 (2026-08-06 사용자 지시)

> "앞으로는 피그마, 웹, 크롬 피그마 등 사용할 수 있는 모든 도구로 할 수 있는 건 **무조건 크롬 피그마로** 다 해.
>  이건 현재 열린, 그리고 앞으로 열릴 **모든 세션들에서 강력하게 필수로** 지켜."

**크롬 피그마로 할 수 있는 피그마 작업은 전부 크롬 피그마로 한다.** `use_figma` 플러그인은
**기본 도구가 아니다.** 편하다는 이유로 플러그인을 먼저 잡지 말 것.

| 작업 | 도구 |
|---|---|
| 문구 수정·노드 생성/삭제·폭 조정·폰트 굵기·텍스트 스타일 생성 | **크롬 피그마** |
| 화면 확인·줄바꿈 판정 | **크롬 피그마** (렌더가 진짜 Pretendard 다) |
| `use_figma` 플러그인 | **읽기 검증 전용**(id·지오메트리·스냅샷 덤프), 그리고 **크롬이 연결 안 됐을 때의 차선책** |

- 크롬 확장이 안 붙어 있으면 `list_connected_browsers` 가 **빈 배열**을 준다. 그때는
  **확장을 켜 달라고 한 줄로 요청하고 기다린다.** 조용히 플러그인으로 우회하지 않는다.
- 플러그인으로 우회해야만 하는 상황이면 **왜 그런지 밝히고** 진행한다.
- 크롬 피그마에서 한글을 타이핑할 때는 **종성 깨짐**을 조심한다. 타이핑 후 `use_figma` 읽기로
  `characters` 를 대조해 확인한다(읽기는 폰트 로드가 필요 없다).

**왜 이 규칙인가**: 플러그인 실행 환경에는 Pretendard 가 없다. 그래서 ①문구 수정이 Gothic A1
경유라는 우회로를 타야 하고 ②줄바꿈·폭 판정이 대체 폰트 기준이라 틀린다(2026-08-06 에 실제로
"제목이 2줄" 이라는 오보를 냈다). 크롬 피그마는 사용자 로컬 폰트를 그대로 쓰므로 둘 다 안 생긴다.
