 ...
 * 이 규칙이 의미가 있다.
 */
const PRICE_OK = new Set([
  // 2026-09-05 — 멤버십·결제 화면이 생겼다. 「가격 표기 원칙」이 말하는 '금액을 말하는 곳 하나'다.
  // ⚠️ 이 검사는 매칭 전에 <script> 를 떼므로, 금액을 JS 로 옮기면 이 목록에 없어도 안 걸린다.
  //    그래서 면제를 **여기 적어 둔다** — 안 적으면 '검사가 못 보는 것'과 '면제한 것'이 구별되지 않는다.
  //    목록이 둘이 되면 그때 다시 생각한다(docs/기획/가격-표기-원칙.md).
  'mypage/billing/index.html',
]);
