...
- **데스크톱 카드 폭 512px / 콘텐츠 464px 을 피그마 360px 에 "맞추지" 않는다.** (2026-08-02 사용자 확정, 이전 400/352)
  피그마 프레임은 360px 고정이고, 데스크톱 폭은 **피그마와 무관한 웹 전용 반응형 값**이다.
  근거: 레퍼런스 실측 — 주선왕 카드 512·글 512 / 피클플러스 카드 540·글 508. 우리는 이전에 글이 352px 이라 31% 좁았다.
- **데스크톱 오버라이드(`@media (min-width:520px)`)는 파일 맨 아래 한 블록에 모아 둔다.**
  기본 규칙보다 위에 두면 같은 특이도에 밀려 `section > *` 같은 오버라이드가 **조용히 무시된다.**
  `desk-override-order` 잠금값과 `width-desk-content-464` 가 둘 다 이걸 지킨다.
