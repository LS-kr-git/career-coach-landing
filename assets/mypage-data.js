 ...
 *   🔴 배선할 때 실패 갈래를 반드시 만든다 (2026-09-17 검사관 ①). 지금 화면은 값이 없으면
 *   EMPTY 를 그리므로, 조회가 실패해 null 이 와도 「아직 배선 안 됨」 과 똑같이 보인다 —
 *   결제한 사람에게 「등록된 결제수단 없음」 을 보여 주게 된다. 실패는 실패로 드러내야 한다.
...
/* 🔴 사람 정보는 목값으로 만들지 않는다 (2026-09-17 사용자 지시 ③).
   ...
   카드사 심사 불가로 보는 것과 같은 신호). 실제 값은 배선(구독 응답) 뒤에야 알 수 있으므로
   **그때까지는 빈 상태를 그린다.** 다른 이름을 넣는 것은 가짜 사람을 한 명 더 만드는 것이다.
...
/* 값이 없다 = 아직 배선되지 않았다. null 을 문구로 쓰지 않고 화면이 EMPTY 를 그린다. */
const PROFILE = { name: null, initial: null, provider: null, email: null, insightCount: null };
